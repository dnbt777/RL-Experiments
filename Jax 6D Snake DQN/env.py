...
import jax
import time
from custom_types import GameStateBatch, TransitionStepBatch
## Improvements:
...
@functools.partial(jax.jit, static_argnames=["height", "width", "apple_count", "batch_size"])
def init_game_state_batch(
  ...
  batch_keys = jrand.split(key, batch_size)
  ...
  snake_tail = -jnp.ones((batch_size, max_snake_len - 1, 2), dtype=int)
  ...
  state_batch = GameStateBatch(
    apples = apples,
    score = jnp.zeros((batch_size,), dtype=int),
    snake_tail = snake_tail,
    snake_tail_length = jnp.zeros((batch_size,), dtype=int),
...
def update_game_state_batched(
  ...
  reward_batch = jnp.ones_like(action_batch, dtype=jnp.float16)*survival_reward # for surviving
  ...
  # update reward
  apple_reward = 1
  reward_batch = reward_batch + apple_reward*snake_ate_apple
  ...
  reward_batch = reward_batch - death_punishment*snake_died
